...
export function addEventListeners() {
	MENU.Profile.addEventListener('click', async (e) => {
...
export async function profile_page() {
	...
	html += `
        <div class="alert alert-primary">
            Email: ${currentUser.email} (Cannot change email as login name)
        </div>
    `;
	...
	html += `
        <form id="form-update-profile" method="post">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="creditNo" value="${accountInfo.creditNo}" disabled required
                                ...
                                pattern="[0-9]+" minlength="15" maxlength="16">
                        </td>
                    </tr>
                </tbody>
            </table>
            <div>
                ...
                <button type="submit" class="btn btn-outline-secondary" style="display: none;" onclick="this.form.submitter='CANCEL'" formnovalidate="true">Cancel</button>
            </div>
        </form>
    `;
