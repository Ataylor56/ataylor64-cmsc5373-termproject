...
export function addEventListeners() {
	MENU.Purchases.addEventListener('click', async (e) => {
...
export async function purchases_page() {
	...
	for (let i = 0; i < carts.length; i++) {
		html += `
                <tr>
                    ...
                    <td>
                        ${new Date(carts[i].timestamp).toString()}
                    </td>
                </tr>
            `;
	}
	...
	for (let j = 0; j < detailsForm.length; j++) {
		detailsForm[j].addEventListener('submit', (e) => {
			e.preventDefault();
...
function buildTransactionView(cart) {
	...
	cart.items.forEach((p) => {
		html += `
            ...
            <tr>
                <td> <img width="95%" src="${p.imageURL}"> </td>
                <td> ${p.brand} ${p.model} ${p.productStyle} </td>
